...
class Application:
    ...
    def perform(self, perf: PerformanceType):
        ...
        for lang in self.langs:
            ...
            for meta in file_meta:
                datafile = f"{workdir}/{meta['file']}"
                backfile = f"{workdir}/b_{meta['file']}"

            match perf:
                case PerformanceType.STANDARD:
                    self.standard_perfile(meta, datafile, backfile)
                case PerformanceType.RESTORE:
                    self.restore_perfile(meta, datafile, backfile)
                case _:
        ...
        self.log("You will need to restart Risk of Rain 2 if it is open.")
    ...
    def restore_perfile(self, meta: dict, datafile: str, backfile: str):
        """
        Restore from a backup for either Equipment or Items.

        `meta`: An item from `file_meta`.

        `datafile`: The path for localization file.

        `backfile`: The path for the backup file created by r2loctool.
        """
        # try to read backup... skip if can't.
        try:
            with open(backfile, 'r') as file:
                self.log(f"Reading b_{meta['file']}...")
                backup = file.readlines()
        except FileNotFoundError:
            self.log(
                f"Could not find b_{meta['file']}, skipping...")
            return

        # write backup to localization file
        self.log(f"Writing to {meta['file']}...")
        ...
        os.remove(backfile)
        self.log(f"{meta['file']} completed successfully.")

    def standard_perfile(self, meta: dict, datafile: str, backfile: str):
        """
        ...
        try:
            with open(datafile, 'r') as file:
                self.log(f"Reading {meta['file']}...")
                data = file.read()
        except FileNotFoundError:
            self.log(
                f"Could not find {meta['file']}, skipping...")
            return

        # remove old backups
        if os.path.isfile(backfile):
            self.log(f"Removing old {meta['file']} backup...")
            os.remove(backfile)

        # create a new one
        self.log(
            f"Creating a backup {meta['file']} before continuing...")
        with open(backfile, 'w') as backup:
            backup.write(data)

        # find entry names and strings
        self.log(
            f"Finding pickup and description entries for {meta['pre']}s...")
        ...
        self.log(f"Processing replacements for {meta['pre']}s...")
        for k in dataPick:
